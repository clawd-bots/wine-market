          ...
          <nav className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-sm font-medium text-earth hover:text-wine transition-colors"
              >
                {link.label}
              </Link>
            ))}
          ...
          <nav className="md:hidden pb-4 border-t border-earth-light/30 pt-4 flex flex-col gap-3">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-sm font-medium text-earth hover:text-wine transition-colors px-2 py-1"
